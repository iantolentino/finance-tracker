        ...
        >
          ...
          <div className="p-8 space-y-6">
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-10 py-2.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-slate-50/50"
                  />
                  <button
                    type="button"
              ...
              <button
                type="submit"
                className="w-full py-2.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow transition"
              >
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row relative">
      ...
      <div className="md:hidden flex items-center justify-between bg-white dark:bg-slate-900 text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 p-4 shrink-0 print:hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {settings.theme === "dark" ? <Sun className="w-5 h-5 text-amber-500" /> : <Moon className="w-5 h-5 text-indigo-500" />}
          </button>
          <button
      ...
      {/* Persistent Sidebar drawer navigation */}
      <AnimatePresence>
        {(mobileMenuOpen || !window.matchMedia("(max-w: 768px)").matches) && (
          <motion.aside
            initial={{ x: -260 }}
            animate={{ x: 0 }}
            exit={{ x: -260 }}
            transition={{ type: "tween", duration: 0.2 }}
            className="w-[260px] bg-white dark:bg-slate-900 text-slate-900 dark:text-white shrink-0 z-40 flex flex-col border-r border-slate-200 dark:border-slate-800 fixed md:sticky top-0 bottom-0 print:hidden h-full"
          >
            {/* Brand details */}
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-semibold tracking-wide transition-all duration-200 ${
                      currentTab === item.id
                        ? "bg-gradient-to-r from-indigo-600 to-indigo-500 text-white shadow-[0_0_12px_rgba(99,102,241,0.3)] border border-indigo-400/20"
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-2">
              ...
              >
                <div className="flex items-center gap-3">
                  {settings.theme === "dark" ? <Sun className="w-4 h-4 text-amber-500" /> : <Moon className="w-4 h-4 text-indigo-500" />}
                  <span>{settings.theme === "dark" ? "Light Mode" : "Dark Mode"}</span>
                </div>
                <div className={`w-8 h-4 rounded-full p-0.5 transition-colors duration-200 ${settings.theme === "dark" ? "bg-indigo-600" : "bg-slate-200 dark:bg-slate-700"}`}>
              ...
              <div className="bg-slate-100 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-200 dark:border-slate-800/60 flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300">
